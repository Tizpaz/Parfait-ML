...
# The implementation is extended for more than the four parameters considered for logistic regression!
import sys
sys.path.append("./")
import numpy as np
...
def flash_fair_LSR(X_train, y_train, X_valid, y_valid, biased_col, group_0, group_1, n_obj, start_time, algorithm):  # biased_col can be "sex" or "race", n_obj can be "ABCD" or "AB" or "CD"
    ...
    def convert_lsr(index):  # 30 2 2 100 2 2 3
        a = int(index / 4800 + 1)
        b = int(index % 4800 / 2400 + 1)
        c = int(index % 2400 / 1200 + 1)
        d = int(index % 1200 / 12 + 1)
        e = int(index % 12 / 6 + 1)
        f = int(index % 6 / 3 + 1)
        g = int(index % 3 + 1)
        return a, b, c, d, e, f, g

    all_case = set(range(0, 144000))
    ...
    while i < len(modeling_pool):
        ...
        if(algorithm == "LogisticRegression"):
            ...
            if temp[6] == 2 and p3 == 'liblinear':
                ...
                p7 = 'auto'
            model = LogisticRegression(C=p1, penalty=p2, solver=p3, max_iter=p4, dual = p5, fit_intercept = p6, multi_class=p7)
        ...
        try:
            ...
            modeling_pool[i] = random.sample(all_case, 1)[0]
            continue
