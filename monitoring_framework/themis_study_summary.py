...
def create_model(model, dataset, algo):
    ...
    return df_pareto_optimal_max_score_row[["score", "AOD", "themis_group_score", "themis_causal_score"]], df_pareto_optimal_max_AOD_row[["score", "AOD", "themis_group_score", "themis_causal_score"]]
...
def main():
    ...
    for d in datasets:
        for m in models:
            ...
            model_score_acc = model_scores[0]
            model_score_acc['dataset'] = d[0] + ", " + d[1]
            model_score_acc['model'] = m
            model_score_acc['optimal'] = "Score"

            model_score_AOD = model_scores[1]
            model_score_AOD['dataset'] = d[0] + ", " + d[1]
            model_score_AOD['model'] = m
            model_score_AOD['optimal'] = "Fairness"
            score_dataframe = score_dataframe.append(model_score_AOD, ignore_index=True)
            score_dataframe = score_dataframe.append(model_score_acc, ignore_index=True)
            count +=1
            my_bar.progress(count/(len(datasets)*len(models)))
